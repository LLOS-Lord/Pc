...
@property (nonatomic, strong) UISegmentedControl *hitboxSeg;
@property (nonatomic, strong) UISegmentedControl *triggerSeg;
...
- (void)setupMenu {
    ...
    // ─── Aimbot section ───
    UILabel *sep = [[UILabel alloc] initWithFrame:CGRectMake(btnX, btnY+2, btnW, 14)];
    sep.text = @"── AIMBOT ──"; sep.textAlignment = NSTextAlignmentCenter;
    sep.textColor = [UIColor colorWithWhite:0.5 alpha:1];
    sep.font = [UIFont systemFontOfSize:10];
    [_scrollView addSubview:sep];
    btnY += 18;
    
    _aimbotButton = [self createButtonWithTitle:@"Aimbot" frame:CGRectMake(btnX, btnY, btnW, btnH)];
    [_aimbotButton addTarget:self action:@selector(toggleAimbot) forControlEvents:UIControlEventTouchUpInside];
    [_scrollView addSubview:_aimbotButton];
    btnY += btnH + btnGap;
    
    // FOV slider
    _fovLabel = [[UILabel alloc] initWithFrame:CGRectMake(btnX, btnY, btnW, 16)];
    _fovLabel.text = [NSString stringWithFormat:@"FOV Size: %.0f", Vars.AimFov];
    _fovLabel.textColor = [UIColor colorWithWhite:0.7 alpha:1];
    _fovLabel.font = [UIFont systemFontOfSize:11];
    [_scrollView addSubview:_fovLabel];
    btnY += 18;
    
    _fovSlider = [[UISlider alloc] initWithFrame:CGRectMake(btnX, btnY, btnW, 22)];
    _fovSlider.minimumValue = 50;
    _fovSlider.maximumValue = 400;
    _fovSlider.value = Vars.AimFov;
    _fovSlider.tintColor = COLOR_ACCENT;
    [_fovSlider addTarget:self action:@selector(fovChanged:) forControlEvents:UIControlEventValueChanged];
    [_scrollView addSubview:_fovSlider];
    btnY += 28;
    
    // Hitbox segmented
    UILabel *hl = [[UILabel alloc] initWithFrame:CGRectMake(btnX, btnY, btnW, 14)];
    hl.text = @"Hitbox"; hl.textColor = [UIColor colorWithWhite:0.55 alpha:1];
    hl.font = [UIFont systemFontOfSize:10];
    [_scrollView addSubview:hl];
    btnY += 16;
    
    _hitboxSeg = [[UISegmentedControl alloc] initWithItems:@[@"Head",@"Neck",@"Body"]];
    _hitboxSeg.frame = CGRectMake(btnX, btnY, btnW, 26);
    _hitboxSeg.selectedSegmentIndex = Vars.AimHitbox;
    _hitboxSeg.tintColor = COLOR_ACCENT;
    [_hitboxSeg addTarget:self action:@selector(hitboxChanged:) forControlEvents:UIControlEventValueChanged];
    [_scrollView addSubview:_hitboxSeg];
    btnY += 32;
    
    // Trigger segmented
    UILabel *tl = [[UILabel alloc] initWithFrame:CGRectMake(btnX, btnY, btnW, 14)];
    tl.text = @"Trigger"; tl.textColor = [UIColor colorWithWhite:0.55 alpha:1];
    tl.font = [UIFont systemFontOfSize:10];
    [_scrollView addSubview:tl];
    btnY += 16;
    
    _triggerSeg = [[UISegmentedControl alloc] initWithItems:@[@"Always",@"Fire",@"Scope",@"F+S"]];
    _triggerSeg.frame = CGRectMake(btnX, btnY, btnW, 26);
    _triggerSeg.selectedSegmentIndex = Vars.AimWhen;
    _triggerSeg.tintColor = COLOR_ACCENT;
    [_triggerSeg addTarget:self action:@selector(triggerChanged:) forControlEvents:UIControlEventValueChanged];
    [_scrollView addSubview:_triggerSeg];
    btnY += 32;
...
- (void)updateMenu {
    ...
    NSArray *buttons = @[_boxESPButton, _linesESPButton, _nameButton, _healthButton,
                         _distanceButton, _skeletonButton, _aimbotButton];
    NSArray *states  = @[@(Vars.Box), @(Vars.lines), @(Vars.Name), @(Vars.Health),
                         @(Vars.Distance), @(Vars.skeleton), @(Vars.Aimbot)];
...
// ── Aimbot actions ──
- (void)toggleAimbot { if (Vars.Enable) Vars.Aimbot = !Vars.Aimbot; }

- (void)fovChanged:(UISlider*)s {
    Vars.AimFov = s.value;
    _fovLabel.text = [NSString stringWithFormat:@"FOV Size: %.0f", s.value];
}
- (void)hitboxChanged:(UISegmentedControl*)sc  { Vars.AimHitbox = (int)sc.selectedSegmentIndex; }
- (void)triggerChanged:(UISegmentedControl*)sc { Vars.AimWhen   = (int)sc.selectedSegmentIndex; }
...
// FREE FIRE OFFSETS - UnityFramework (من Hooks.h)
void game_sdk_t::init()
{
    this->GetHp = (int (*)(void *))getRealOffset(0x4A8478C);
    this->Curent_Match = (void *(*)())getRealOffset(0x4E355B0);
    this->GetLocalPlayer = (void *(*)(void *))getRealOffset(0x28FC854);
    this->GetHeadPositions = (void *(*)(void *))getRealOffset(0x4AA1A28);
    ...
    this->WorldToScreenPointEx = (Vector3(*)(void *, Vector3, int))getRealOffset(0x84E6AC8);
    this->get_isVisible = (bool (*)(void *))getRealOffset(0x4A20AF4);
    ...
    this->set_aim = (void (*)(void *, Quaternion))getRealOffset(0x4A1C91C);
    this->get_IsSighting = (bool (*)(void *))getRealOffset(0x4A0FF18);
    this->get_IsFiring = (bool (*)(void *))getRealOffset(0x4A05634);
